...
window.App = {
  start: function() {
    ...
    try {
      ...
      // var currentProvider = new Web3.providers.WebsocketProvider('wss://mainnet.infura.io/ws');
      const web3 = new Web3(currentProvider);
        
      Token.setProvider(web3.currentProvider);
      Sale.setProvider(web3.currentProvider);

      // Set providers is now its own function due to the hack needed to get around web3:1.0.0 http providers
      setProviders();
    }
  ...
  refreshLogs: function() {
    var self = this;
    var sale;
    var tranaction_counter = 0;

    // var currentProvider = new Web3("wss://mainnet.infura.io/ws");
    // const web3_logs = new Web3(currentProvider);
        
    // Token.setProvider(web3_logs.currentProvider);
    // Sale.setProvider(web3_logs.currentProvider);

    // Sale.events.TokenPurchase({
    //   filter: {}, 
    //   fromBlock: 5380238
    ...
    //   // Watch for all events on the contract
    //   var purchaseEvent = sale.TokenPurchase({fromBlock: 5380238, toBlock: 'latest'});

    //   purchaseEvent.watch(function(error, event){
    //     if (!error){
    //       console.log(event);
    //     }
    //   });
    ...
    //   purchaseEvent.get(function(error, logs){
    //     console.log('stop loading');

    //     for(var l in logs){

    //       // var event = logs[l].event;
    //       console.log(logs[l].event);
    ...
    // tranaction_counter = addCommas(tranaction_counter);
    // $('#transaction_num').html(tranaction_counter);
  },
