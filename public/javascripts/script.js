...
(function($){
	...
	if ($count_token.length > 0 ) {
		$count_token.each(function() {
			var $self = $(this);
			var d = new Date(); d.setMinutes(d.getMinutes() + 10);
			var datetime = d;
			$self.countdown(datetime).on('update.countdown', function(event) {
				$(this).html(event.strftime('' + '<div class="col"><span class="countdown-time">%M</span><span class="countdown-text">Minutes</span></div>' + '<div class="col"><span class="countdown-time countdown-time-last">%S</span><span class="countdown-text">Seconds</span></div>'));
			});
		});
